# Copy EN · Gateway `casas-viejas-1933` · 2 February 1933 · Casas Viejas

> [nota de diseño] Traducción de `docs/copy_es/sesiones_casas-viejas-1933.md` según el glosario. Las citas (claves
> `.cita.`) quedan en español; su traducción, en `_trad_pendientes.txt`.

<!-- sesiones.casas-viejas-1933.meta.titulo -->
Casas Viejas · 2 February 1933

<!-- sesiones.casas-viejas-1933.meta.descripcion -->
Azaña answers in the Constituent Cortes for the events at Casas Viejas: what the Diario says and how to follow the village's name in the explorer.

<!-- sesiones.casas-viejas-1933.antetitulo -->
2 February 1933 · Session no. {{sesion.1933-02-02-288.num|id}} · Constituent · DSCCRE no. {{sesion.1933-02-02-288.diario_num|id}}, pp. {{sesion.1933-02-02-288.paginas}} (project metadata)

<!-- sesiones.casas-viejas-1933.titulo -->
Casas Viejas

<!-- sesiones.casas-viejas-1933.entrada -->
What happened in this session, and how do I read it in full?

## What happened

<!-- sesiones.casas-viejas-1933.que_paso.texto -->
Between 28 December 1932 and 1 February 1933 the Cortes do not meet: the numbers of the Diario run on without a gap.

<!-- sesiones.casas-viejas-1933.que_paso.2 -->
In its second session of February, Manuel Azaña, President of the Council of Ministers, answers for the events at Casas Viejas.

<!-- sesiones.casas-viejas-1933.que_paso.3 -->
He denies any responsibility on the part of the Government, and the minorities protest. Three weeks later, Martínez Barrio reminds him of his words in the Chamber.

<!-- sesiones.casas-viejas-1933.que_paso.4 -->
The village's name comes back again and again: the search finds it from February 1933 to July 1936.

## What the Diario says

<!-- sesiones.casas-viejas-1933.diario.cita.hurgue -->
«…por mucho que se hurgue no se encontrará un atisbo de responsabilidad para el Gobierno.»

<!-- sesiones.casas-viejas-1933.diario.cita.hurgue.pie -->
Manuel Azaña · {{cita.casasviejas.azana.hurgue.palabras}} words

<!-- sesiones.casas-viejas-1933.diario.cita.azana -->
«En Casas Viejas no ha ocurrido sino lo que tenía que ocurrir. (Fuertes rumores y protestas en los bancos de las minorías; contraprotestas en la mayoría.)»

<!-- sesiones.casas-viejas-1933.diario.cita.azana.pie -->
Manuel Azaña, in the same row, with the shorthand writer's bracketed note (acotación)

<!-- sesiones.casas-viejas-1933.diario.cita.martinezbarrio -->
«…nos dijo: “En los sucesos de Casas Viejas, Sres. Diputados, por mucho que se hurgue…”»

<!-- sesiones.casas-viejas-1933.diario.cita.martinezbarrio.pie -->
Diego Martínez Barrio, 23 February 1933

## The session, turn by turn

<!-- sesiones.casas-viejas-1933.turnos.texto -->
The tallest row is Azaña's; he speaks several times in the session.

## How to find it in the explorer

<!-- sesiones.casas-viejas-1933.explorador.fecha -->
«Filtros › Fecha y sesión»: «Desde» and «Hasta» 02/02/1933. This gives {{sesion.1933-02-02-288.filas_v3}} interventions in the explorer; with «Solo lo que se habla», {{sesion.1933-02-02-288.habla_v3}}.

<!-- sesiones.casas-viejas-1933.explorador.consulta -->
"casas viejas"

<!-- sesiones.casas-viejas-1933.explorador.recuento -->
Count of {{busquedas.fecha|fecha_larga}}: {{busqueda.casas_viejas.n}} interventions in the explorer.

<!-- sesiones.casas-viejas-1933.explorador.consulta.meses -->
Counting all rows, there are {{busqueda.casas_viejas.m1933_02}} interventions in February 1933 and {{busqueda.ses.casas_viejas.m1933_03}} in March.

<!-- sesiones.casas-viejas-1933.explorador.consulta.meses.habla -->
With «Solo lo que se habla», which leaves out summaries and comments, there are {{busqueda.casas_viejas.m1933_02_habla}} and {{busqueda.ses.casas_viejas.m1933_03_habla}}.

<!-- sesiones.casas-viejas-1933.explorador.consulta.frase -->
Azaña's phrase, in quotation marks, appears in {{busqueda.ses.tenia_que_ocurrir.n|letra}} interventions: his own and Martínez Barrio's.

<!-- sesiones.casas-viejas-1933.explorador.frase.consulta -->
"no ha ocurrido sino lo que tenía que ocurrir"

<!-- sesiones.casas-viejas-1933.explorador.frase.recuento -->
Count of {{busquedas.fecha|fecha_larga}}: {{busqueda.ses.tenia_que_ocurrir.n|letra}} interventions in the explorer.

<!-- sesiones.casas-viejas-1933.explorador.biblioteca -->
The ready-made debate «Casas Viejas» gathers {{bib.casas_viejas.entradas}} interventions from {{bib.casas_viejas.sesiones|letra}} sessions, in February and March 1933.

## What is not there

<!-- sesiones.casas-viejas-1933.no_esta.texto -->
Searched for in the corpus, «tiros a la barriga» (‘shots to the belly’) does not appear in 1933. The first row to contain it is a shout of 31 May 1934:

<!-- sesiones.casas-viejas-1933.no_esta.cita.barriga -->
«Casas Viejas! Casas Viejas! Tiros a la barriga, a la barriga!»

<!-- sesiones.casas-viejas-1933.no_esta.cita.barriga.pie -->
Several deputies, in a bracketed note in the row of José Tomás y Piera

<!-- ↺ comun.fija.contar -->
Counting a word does not tell you who defends it or in what tone.

## Sources

<!-- sesiones.casas-viejas-1933.fuentes.texto -->
Everything on this page comes from the Diario de Sesiones, read in the dataset. No fact is stated here that the Diario does not record.

## How to cite a passage

<!-- sesiones.casas-viejas-1933.citar.texto -->
Give the Diario, its number, the date and the pages. If you work with the dataset, add the row id.

<!-- sesiones.casas-viejas-1933.citar.cita -->
Diario de Sesiones de las Cortes Constituyentes de la República Española, no. {{sesion.1933-02-02-288.diario_num|id}}, 2 February 1933, pp. {{sesion.1933-02-02-288.paginas}} (project metadata). Luz y Taquígrafos, row {{cita.casasviejas.azana.V2|id}}.

<!-- ↺ comun.fija.diario -->
A derived edition for research: wherever it differs, the *Diario de Sesiones* prevails.
